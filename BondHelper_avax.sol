...
interface IWhiteLogic{
    function clickDepositHelper(address _to,uint256 _payout ) external returns(bool);
}
...
contract DepositAvaxHelper is Ownable {
    ...
    constructor (address _deposit,address _ICE, address _WAVAX, ITreasury _treasury) {
    ...
    function depositHelper(
        ...
        IWhiteLogic(whiteLogic).clickDepositHelper(msg.sender,payout);
        return payout;
    }
    ...
    function setWhiteLogic(address _whiteLogic) external onlyPolicy returns (bool) {
        whiteLogic = _whiteLogic;
        return true;
    }
