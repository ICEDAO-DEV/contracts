...
contract wavaxBondDepository is Ownable {
    ...
    address public immutable ICE; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints ICE when receives principle
    ...
    struct Bond {
        uint256 payout; // ICE remaining to be paid
    ...
    function deposit(
        ...
        uint256 priceInBNB = bondPriceInBNB();
        // Stored in bond info
        ...
        // must be > 0.0001 ICE ( underflow protection )
        ...
        /**
                ...
                approved and
                deposited into the treasury, returning (_amount - profit) ICE
             */
        ...
        if (fee != 0) {
            ...
            IERC20(ICE).safeTransfer(DAO, fee);
        }
        ...
        emit BondPriceChanged(bondPriceInBNB(), _bondPrice(), debtRatio());
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(ICE).transfer(_recipient, _amount);
            // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(ICE).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(ICE).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(ICE).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function assetPrice() public view returns (uint256) {
        (uint256 reserve0, uint256 reserve1,) = IPancakePair(mim_wavax_lp).getReserves();
        return reserve0.mul(1e18).div(reserve1); //109 * 1e18
    }
     ...
     *  @return price_ uint
     */
    function bondPriceInBNB() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice()
            ...
            price_ = bondPrice().mul(assetPrice()).div(100); //   wavax_ice * wavax_mim = 
            
        }
    }

    /**
     *  @notice calculate current ratio of debt to ICE supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(ICE).totalSupply();
    ...
    /**
     *  @notice calculate amount of ICE available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or ICE) to the DAO
     *  @return bool
     */
